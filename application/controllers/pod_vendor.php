...
class pod_vendor extends CI_Controller {
    ...
    public function index() {
        if ($this->session->userdata('sess_apps') == TRUE) {
            ...
            $data['STR_PAGE']       = 'PAGE_POD_VENDOR';
            $data['TITLE']          = 'USER LOGIN';
            $data['SUB_TITLE']      = 'POD Via - Vendor Status';
			...
			$query = "select mswb_nomor, mswb_status, tvve_tanggal, lowb_kode from mst_waybill
			left join trs_via_vendor on (tvve_mswb_nomor = mswb_nomor)
			left join trs_status_waybill on tswb_mswb_nomor = mswb_nomor
			left join log_waybill on lowb_mswb_nomor = mswb_nomor
			where tvve_status_proses = '1' and tvve_mven_kode = '".$data['vendor']."' and lowb_kode = 'VDR'
			and tswb_mswb_nomor is null";
    ...
    public function formadd() {
        if ($this->session->userdata('sess_apps') == TRUE) {
            ...
            $data['STR_PAGE']       = 'PAGE_POD_VENDOR_ADDFORM';
            $data['TITLE']          = 'Add New';
    ...
    public function getValidationTime() {
    	if ($this->session->userdata('sess_apps') == TRUE) {
    		...
    		$format_jam_pod = mysql_real_escape_string($this->input->post('jam_pod'));
    		
    		if ($format_jam_pod == "")
			{ $format_jam_pod = "undefined";}

    		$rex = "regex_match[/(2[0-4]|[01][1-9]|10):([0-5][0-9])/";
    		$data = $this->form_validation->set_rules('jam_pod', 'Contact No.', 'required|regex_match[/(2[0-4]|[01][1-9]|10):([0-5][0-9])/');
    		if (preg_match($rex) || $data->num_rows() >0) {
    			echo "Format Jam Benar";
				return;
    		}
    		else
			{
    ...
    public function savedata() {
        if ($this->session->userdata('sess_apps') == TRUE) {
			...
			$nomor = mysql_real_escape_string($this->input->post('waybill'));
			$m_waybill= explode("|",$nomor);
			$typeWaybillNomor = $m_waybill[0];
			$typeWaybillDate = $m_waybill[1];
			...
			$jam_pod = date('H:i:s',strtotime(mysql_real_escape_string($this->input->post('jam_pod'))));
			$vanTime = strtotime("-120 minutes", strtotime($jam_pod));
			$jam_van = date('H:i:s',$vanTime);
			$sipTime = strtotime("-135 minutes", strtotime($jam_pod));
			...
			$queryTuju = "select ttwb_nomor, ttwb_mcus_mcty_kode, ttwb_mcus_kota, ttwb_mcus_mkcm_kode, ttwb_mcus_kecamatan, ttwb_mpen_mcty_kode, ttwb_mpen_kota, ttwb_mpen_mkcm_kode, ttwb_mpen_kecamatan from trs_transaksi_waybill where ttwb_nomor= '".$typeWaybillNomor."'";
			...
			$queryPOD = "INSERT INTO log_waybill (lowb_pk
                                ...
                                select f_get_pk() pk
                                				...
                                				, '".$vendorCode."'
                                				, '".$vendorName."' 
                                        from mst_waybill left join trs_via_vendor on tvve_mswb_nomor = mswb_nomor and tvve_mven_kode = '".$vendor."' where mswb_nomor = '".$typeWaybillNomor."'";
			...
			{
				echo "<script>
                        alert('Gagal insert POD Vendor status.');
                        window.history.back();
                      </script>";
				return;
			}
			...
			{
				echo "<script>
                        alert('Gagal Insert Log POD Vendor.');
                        window.history.back();
                      </script>";
				return;
			}
			...
			echo "<script>
                        alert('set status POD Vendor sukses.');
    ...
    public function formUpdate($LOCK_ID) {
        if ($this->session->userdata('sess_apps') == TRUE) {
            ...
            $data['STR_PAGE']       = 'PAGE_POD_VENDOR_FORMUPDATE';
